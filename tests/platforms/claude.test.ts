...
import { deriveClaudeTitle, extractClaudeConversation, isEligibleClaudeConversation, prepareClaudeConversationForExport } from "@/platforms/claude";
...
describe("extractClaudeConversation", () => {
	...
	test("preserves Claude thinking summaries ahead of assistant content", () => {
			...
			createElement("div", { class: "grid grid-rows-[auto_auto] min-w-0" }, [
				createElement("div", { class: "row-start-1 col-start-1 min-w-0" }, [
					createElement("div", { class: "min-w-0 pl-2 py-1.5" }, [
						createElement(
							...
							[createElement("div", { class: "inline-flex items-center gap-1 min-w-0" }, [createElement("span", { class: "truncate text-sm font-base" }, ["Prepared final response"])])],
						),
						createElement("span", { class: "sr-only", role: "status", "aria-live": "polite" }, ["Prepared final response"]),
						createElement(
							"div",
							{
										...
										createElement("div", undefined, [
											createElement(
												...
												[
													...
													createElement("ol", undefined, [createElement("li", undefined, ["First item"]), createElement("li", undefined, ["Second item"])]),
												],
											),
										]),
				...
				createElement("div", { class: "row-start-2 col-start-1 relative grid isolate min-w-0" }, [
					createElement("div", { class: "row-start-1 col-start-1 relative z-[2] min-w-0" }, [
						createElement("div", undefined, [
							createElement("div", { class: "standard-markdown grid-cols-1 grid gap-3 standard-markdown" }, [createElement("p", undefined, ["Final response text"])]),
						]),
					]),
				]),
			]),
		...
		expect(messages[0]?.markdown.indexOf("Prepared final response")).toBeLessThan(messages[0]?.markdown.indexOf("The user wants two things:") ?? Number.MAX_SAFE_INTEGER);
		expect(messages[0]?.markdown.indexOf("The user wants two things:")).toBeLessThan(messages[0]?.markdown.indexOf("Final response text") ?? Number.MAX_SAFE_INTEGER);
	});
	...
	test("extracts artifact cards from Claude open artifact buttons", () => {
		...
		const assistant = createClaudeAssistantMessage(
			createElement("div", undefined, [
				createElement("div", { class: "standard-markdown grid-cols-1 grid gap-3 standard-markdown" }, [createElement("p", undefined, ["Artifact summary"])]),
				artifactCard,
			]),
		);
		...
		const assistant = createClaudeAssistantMessage(
			createElement("div", undefined, [
				createElement("div", { class: "overflow-hidden" }, [
					createElement("div", { class: "standard-markdown grid-cols-1 grid gap-3 standard-markdown" }, [createElement("p", undefined, ["Actual assistant response"])]),
				]),
			]),
		);
	...
	test("uses concise attachment labels from Claude file thumbnails", () => {
		const thumbnail = createElement("div", { "data-testid": "file-thumbnail" }, [
			createElement(
				...
				[createElement("p", undefined, ["Context handoff for continuing this discussion on my phone"])],
			),
		]);

		const wrapper = createElement("div", { "data-test-render-count": "1" }, [thumbnail, createClaudeUserMessage("Attached a handoff note.")]);
	...
	test("ignores orphan claude-response fallback nodes outside message wrappers", () => {
		container.appendChild(createElement("div", { class: "font-claude-response" }, [createElement("p", undefined, ["[HAN]"])]));
	...
	test("expands collapsed Claude thinking panels before export", async () => {
		...
		toggle.addEventListener("click", () => {
			...
			detailRoot.appendChild(createElement("div", { class: "standard-markdown grid-cols-1 grid gap-3 standard-markdown" }, [createElement("p", undefined, ["The user wants two things:"])]));
		});

		const assistant = createElement("div", { "data-is-streaming": "false", class: "group relative" }, [
	...
	test("strips thinking-panel favicon and link noise while keeping narrative text", () => {
								...
								createElement("div", { class: "overflow-hidden min-w-0" }, [
									createElement(
										...
										[
											...
											createElement("p", undefined, ["Key finding: WhatsApp Cloud API requires a dedicated phone number."]),
		...
		const messages = extractClaudeConversation();
		expect(messages).toHaveLength(1);
		expect(messages[0]?.markdown).toContain("Now let me research the WhatsApp Cloud API webhook requirements.");
		expect(messages[0]?.markdown).toContain("Key finding: WhatsApp Cloud API requires a dedicated phone number.");
		expect(messages[0]?.markdown).toContain("Final response text");
