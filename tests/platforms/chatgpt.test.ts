...
import { createElement, setWindowLocation } from "@tests/helpers";

import { deriveChatGPTTitle, extractChatGPTConversation, isEligibleChatGPTConversation, prepareChatGPTConversationForExport } from "@/platforms/chatgpt";

function createStandardTurn(index: number, role: "user" | "assistant", children: Element[]): HTMLElement {
	return createElement("article", { "data-testid": `conversation-turn-${index}` }, [createElement("div", { "data-message-author-role": role }, children)]);
}

function createFallbackTurn(index: number, role: "user" | "assistant", children: Element[]): HTMLElement {
	return createElement("article", { "data-testid": `conversation-turn-${index}`, "data-turn": role }, children);
}

function setLocation(hostname: string, pathname: string): void {
	setWindowLocation({ hostname, pathname });
}
...
describe("extractChatGPTConversation", () => {
	...
	test("extracts user and assistant turns in order", () => {
		container.appendChild(createStandardTurn(0, "user", [createElement("div", { class: "whitespace-pre-wrap" }, ["First prompt"])]));
		container.appendChild(createStandardTurn(1, "assistant", [createElement("div", { class: "markdown" }, [createElement("p", undefined, ["First answer"])])]));
	...
	test("falls back to data-turn when author-role wrappers are absent", () => {
		container.appendChild(createFallbackTurn(0, "user", [createElement("div", { class: "whitespace-pre-wrap" }, ["Fallback prompt"])]));
		container.appendChild(
			createFallbackTurn(1, "assistant", [createElement("div", { class: "prose" }, [createElement("p", undefined, ["Fallback ", createElement("strong", undefined, ["answer"])])])]),
		);
	...
	test("deduplicates nested assistant markdown blocks", () => {
		const nestedMarkdown = createElement("div", { class: "markdown" }, [createElement("p", undefined, ["Only once"])]);
		const duplicateMarkdown = createElement("div", { class: "markdown" }, [createElement("p", undefined, ["Only once"])]);

		container.appendChild(createStandardTurn(0, "assistant", [createElement("div", { class: "prose" }, [nestedMarkdown]), duplicateMarkdown]));
	...
	test("keeps markdown prose blocks that are themselves markdown roots", () => {
		container.appendChild(
			createFallbackTurn(0, "assistant", [
				createElement("div", { "data-message-author-role": "assistant" }, [createElement("div", { class: "markdown prose" }, [createElement("p", undefined, ["Planning chunk"])])]),
				createElement("div", { "data-message-author-role": "assistant" }, [createElement("div", { class: "markdown prose" }, [createElement("p", undefined, ["Final answer chunk"])])]),
			]),
		);
		...
		expect(messages[0]?.markdown.indexOf("Planning chunk")).toBeLessThan(messages[0]?.markdown.indexOf("Final answer chunk") ?? -1);
	});
	...
	test("formats user attachments and assistant artifacts as markdown sections", () => {
		...
		const assistantTurn = createStandardTurn(1, "assistant", [
			createElement("div", { class: "markdown" }, [createElement("p", undefined, ["Looks consistent."])]),
	...
	test("extracts markdown-only assistant blocks when no turns exist", () => {
		container.remove();
		document.body.appendChild(createElement("div", { class: "markdown" }, [createElement("p", undefined, ["Standalone markdown block"])]));
...
describe("prepareChatGPTConversationForExport", () => {
	test.serial("cancels partial exports and restores the original scroll position", async () => {
		...
		const first = createFallbackTurn(0, "user", [createElement("div", { class: "whitespace-pre-wrap" }, ["Synthetic prompt"])]);
