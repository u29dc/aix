...
describe('composeMarkdown', () => {
	test('creates markdown with title as h1', () => {
		...
		const result = composeMarkdown(messages, 'Test Chat', 'claude', 'Claude', 'https://claude.ai/chat/123');
	...
	test('includes URL in metadata', () => {
		...
		const messages: Message[] = [{ role: 'user', markdown: 'Hello' }];
		const result = composeMarkdown(messages, 'Test', 'claude', 'Claude', url);
	...
	test('uses default title when empty', () => {
		const messages: Message[] = [{ role: 'user', markdown: 'Hello' }];
		const result = composeMarkdown(messages, '', 'claude', 'Claude', 'https://claude.ai/chat/123');
