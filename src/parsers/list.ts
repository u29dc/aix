...
function collectListItemMarkdown(
	item: Element,
...
export function formatList(
	...
	for (const child of Array.from(listElement.children)) {
		if (child.tagName?.toLowerCase() === 'li') {
			...
			const content = normalizeTaskMarkerSpacing(
				collectListItemMarkdown(child, nextContext, convertNode),
			);
			const lines = content.split('\n').filter((line) => line.trim().length > 0);
			const firstLine = `${indent}${value}${lines.shift() ?? ''}`;
			const extraLines = lines.map((line) =>
				isListMarkerLine(line) ? line : `${indent}${continuation}${line}`,
			);
