...
const HIDDEN_CONTENT_TYPES = new Set(["execution_output", "model_editable_context", "reasoning_recap", "thoughts", "user_editable_context"]);
...
async function requestJson(fetcher: typeof fetch, input: string, init: RequestInit, label: string): Promise<unknown> {
...
async function fetchConversation(fetcher: typeof fetch, conversationId: string): Promise<Record<string, unknown>> {
...
function resolveFileCitations(value: string, metadataValue: unknown, role: ChatGPTTurnShell["role"]): string | null {
...
function formatParts(parts: unknown, metadata: unknown, role: ChatGPTTurnShell["role"]): string | null {
...
function formatCode(content: Record<string, unknown>): string | null {
	...
	if (!text) return null;
	const language = getNonEmptyString(getField(content, "language"))?.replace(/[^a-z0-9_+#.-]/gi, "") ?? "";
	const fence = pickFence(text);
	return `${fence}${language}\n${text.trimEnd()}\n${fence}`;
}

function formatContent(contentValue: unknown, metadata: unknown, role: ChatGPTTurnShell["role"]): { markdown: string; supported: boolean } {
	...
	switch (contentType) {
		...
		case "tether_browsing_display": {
			const summary = getNonEmptyString(getField(contentValue, "summary")) ?? getNonEmptyString(getField(contentValue, "result"));
			return summary ? { markdown: formatText(summary, role), supported: true } : { markdown: "", supported: false };
		}
		default:
...
function formatAttachmentEntries(metadataValue: unknown): string[] {
	...
	for (const attachment of attachments) {
		...
		const entry = mimeType ? `${escapeMarkdown(name)} (${escapeMarkdown(mimeType)})` : escapeMarkdown(name);
...
function formatApiMessage(message: ChatGPTApiMessage, role: ChatGPTTurnShell["role"]): FormattedApiMessage {
...
function getConversationMapping(value: Record<string, unknown>): Record<string, ChatGPTApiNode> {
	...
	for (const [key, node] of Object.entries(mappingValue)) {
		if (isRecord(node)) mapping[key] = node;
	}
	return mapping;
}

function buildCurrentBranch(conversation: Record<string, unknown>, mapping: Record<string, ChatGPTApiNode>): BranchNode[] {
...
function formatShellSegment(segment: BranchNode[], shell: ChatGPTTurnShell): Message {
	...
	for (const entry of segment) {
		...
		if (!formatted.supported) {
			throw new ChatGPTApiUnavailableError("ChatGPT API response contained an unsupported visible turn.");
		}
...
export async function extractChatGPTConversationFromApi(options: ChatGPTApiExtractionOptions): Promise<Message[]> {
	...
	for (let index = 1; index < shellIndexes.length; index += 1) {
		if ((shellIndexes[index] ?? -1) <= (shellIndexes[index - 1] ?? -1)) {
			throw new ChatGPTApiUnavailableError("ChatGPT visible turn order did not match the API branch.");
		}
	}
